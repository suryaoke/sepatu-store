<div class="content">
    <form action="{{ route('kontak.update') }}" method="POST">
        @csrf
        <input type="hidden" name="id" value="{{ $kontak->id }}" id="">
                        ...
                        <div class="preview">
                            <div>
                                ...
                                <input id="modal-form-1" type="number" name="no_hp" class="form-control"
                                    placeholder="Masukkan No Hp" value="{{ $kontak->no_hp }}">
                            </div>
                            <div class="mt-2">
                                <label for="modal-form-2" class="form-label">Email</label>
                                <input id="modal-form-2" type="email" name="email" class="form-control"
                                    placeholder="example@gmail.com" value="{{ $kontak->email }}">
                            </div>
                            <div class="mt-2">
                                <label for="modal-form-2" class="form-label">Alamat</label>
                                <textarea class="form-control" name="alamat" id="" cols="20" rows="5"> {{ $kontak->alamat }} </textarea>
                                ...
                                <select class="form-control tom-select" id="province"
                                    ...
                                    @foreach ($provinces as $province)
                                        <option value="{{ $province->id }}"
                                            {{ $province->id == $kontak->province_id ? 'selected' : '' }}>
                                            {{ $province->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control tom-select" name="city_id" required>
                                    ...
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}"
                                            {{ $city->id == $kontak->city_id || $city->id == $selectedCity ? 'selected' : '' }}>
                                            {{ $city->name }}
                                        </option>
                                    @endforeach
                                </select>
